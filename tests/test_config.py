...
def test_get_config():
    assert (
        isinstance(test_config.configuration, dict)
...
def test_set_config():
    ...
    assert (
        config["COLOR_1"] == "Blue"
    ), "Congifuration set value returned is incorrect"
...
def test_load_from_file():
    ...
    ), "Failed to read from file and return a list of lines"
    assert (
        isinstance(read[0], str)
    ), "Data line read from file is not a str"
    assert (
        read[0] == test_read_data
